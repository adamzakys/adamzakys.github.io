...
content = re.sub(r'    #navbar \{.*?    \.nav-inner \{.*?\}', navbar_css, content, flags=re.DOTALL)


# 2. Fix Mobile Nav Tab (Match header rounded corners)
mobile_nav_css = """      #nav-menu {
        position: absolute;
        top: calc(100% + 10px);
        left: 5%;
        width: 90%;
...
content = re.sub(r'      #nav-menu \{.*?      #nav-menu\.open \{.*?\}', mobile_nav_css, content, flags=re.DOTALL)

# Remove empty rule
content = re.sub(r'\s*\.nav-actions\s*\{\s*\}', '', content)
